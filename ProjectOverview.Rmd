...
- By taking into account number of reviews (weighted), and keeping Genre constant, we get more accuracy.
...
```{r}
#Currently you must enter the name of the movie with a quotation mark
...
The **for-loop with same genre** performed best qualitatively, but it is slower (takes about 2-3 seconds for a reccomendation)
...
*See Main.R for the executable code*
...
* `Actor`, `Director` are also useful information, but due to limied time we did not take these into consideration when recommending movies.   
  __But these information goes into our ShinyApp!__
* We also fetch the poster of the movie our customer is searching for using keyworkd `Image`.
